...
double output_pid=0;
int pwm =0;
const int deadzone = 20;  // motor begint pas boven 40


double setpoint_pendulum = 0 ; // Desired angle (vertical position)
double setpoint_pendulum_down = 0 ; 
...
void loop() {
  ...
  if (currentMs - lastMs >= FREE_RUN_PERIOD_MS) {// periodic sampling
    ...
    if (sig_angle_deg<-170){
      
      output=-255;
    }
 
    if (-170>sig_angle_deg<-60){
      output=255;
    }
    if (output > 0) {
...
void readAndPrintAngle() {
      lastMs = currentMs;
      sig_angle_deg = ((float)as5600.readAngle()*0.0879)-100-190-5+2.64-30+28-6.52+12.3-11+47.83;
}