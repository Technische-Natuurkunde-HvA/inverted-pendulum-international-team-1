...
void loop() {
  ...
  if (sig_angle_deg < 5 && sig_angle_deg > -5 ){
    output = 0 ;
  }
  else if (sig_angle_deg > 0 && output > -255){
    output =-255 ;
    delay(600);
  }
  else if (sig_angle_deg < 0 && sig_angle_deg > -25.84 && output > -255){
    output = 255 ;
    delay(600);
  }
...
void readAndPrintAngle() {
      lastMs = currentMs;
      sig_angle_deg = ((float)as5600.readAngle()*0.0879)-144.07+3.08-1.76; //0.0879=360/4096;  // degrees [0..360)
